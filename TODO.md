...
1. ¿Qué puertos exactos deben exponerse en `docker-compose.yml` para `backend` y `web`?
2. ¿Qué variables/env y credenciales exactas de Postgres se deben usar en `docker-compose.yml`?
...
7. ¿Cuál es el endpoint/contrato exacto de refresh token en backend (asumido `/auth/refresh` con `{ refreshToken }`)?
